...
Разработала веб-приложение на Django, представляющее собой блог, где пользователи могут:
...
1. Создала новое приложение:

    ```bash
    python manage.py startapp blog
    ```

2. Зарегистрировала приложение в `settings.py`:
...
Создала модели для постов и комментариев в `models.py` приложения `blog`:
...
1. В `views.py` добавила отображение списка постов:
...
2. Настроила маршруты в `urls.py` приложения `blog`:
...
3. В `urls.py` проекта подключила маршруты приложения `blog`:
...
1. Создала папку `templates/blog`.

2. Добавила файл `post_list.html`:
...
3. Создала файл `base.html` в папке `templates`:
...
1. Добавила функцию поиска в `views.py`:
...
2. Настроила маршрут поиска в `urls.py`:

    ```python
    path('search/', views.post_search, name='post_search'),
    ```

3. В `base.html` добавила форму поиска:
...
1. В `static/blog/styles.css` добавила базовые стили:
...
2. Убедилась, что настройки статических файлов корректны в `settings.py`:
